...
if __name__ == '__main__':
    ...
    # Initialization
    base = wd.World(cam_pos=[-4.3, -3, 2.4], lookat_pos=[0, 0, 0.2], fov=25, w=960, h=1200)
    props = WindowProperties()
    props.set_origin(960, 0)
    base.win.requestProperties(props)

    gm.gen_frame().attach_to(base)
    robot_s = CobottaPro(enable_cc=True)
    component_name = 'arm'
    init_jnts = np.array([0.73862334, 1.54322012, -1.91165913, 2.37207699, -2.19527513, 2.91819051])
    ...
    for i in range(len(candidates)-1):
        ...
        object_copy.set_pos(objpos + [-0.315, 0, 0]) # -0.288
        ...
        pcd.transform(mat1)
        pcd.translate([-0.315, 0, 0]) #-0.288
        gm.gen_pointcloud(pcd.points, [[1, 1, 0, 1]], 3).attach_to(base)
        pcd0 = o3d.io.read_point_cloud('object.ply')
        pcd0.transform(np.linalg.inv(mat2))
        extents0 = pcd0.get_axis_aligned_bounding_box().get_extent()

        pcdx = o3d.io.read_point_cloud('/home/ch/research/irex/obstacle2.ply')
        pcdx.transform(mat1)
        ...
        mesh.translate([-0.315, 0, 0]) #-0.288
        obstacle = cm.CollisionModel(mesh)
        #obstacle.change_cdprimitive_type('point_cloud', expand_radius=0.001)
        obstacle.attach_to(base)
        obstacle_list.append(obstacle)

        obstacle3 = o3d.io.read_triangle_mesh('obstacle3.ply')
        obstacle3 = cm.CollisionModel(obstacle3)
        obstacle3.attach_to(base)
        ...
        for index in range(len(tcp)):
            ...
            jawcenter = objrotmat.dot(jaw[index]) + objpos + [-0.315, 0, 0]
            ...
            start_jaw_center_pos = objrotmat.dot(gripos) + objpos + [-0.315, 0, 0] #-0.288
            ...
            while j < 50:
                j += 1
                goal_obj_pos = [-0.1, 0.6, -0.15 + extents0[2] / 2] #-0.14
    ...
    print(approach1)
    np.savetxt('/home/ch/catkin_ws/src/cobotta_tutorials/scripts/rearrange.txt', approach2[0])
    np.savetxt('/home/ch/catkin_ws/src/cobotta_tutorials/scripts/grasp2.txt', np.concatenate([middle1_out, approach1]))
    np.savetxt('/home/ch/catkin_ws/src/cobotta_tutorials/scripts/place2.txt', np.concatenate([middle2_out, approach2]))
