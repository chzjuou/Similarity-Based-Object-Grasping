...
if __name__ == '__main__':
    ...
    # rotmat = rm.rotmat_from_axangle([0, 1, 0], math.pi)
    # jnts = robot_s.ik(component_name, pos, rotmat)
    # print(np.around(jnts / math.pi * 180, 2))
    # robot_s.fk(component_name, jnts)
    # robot_s.gen_meshmodel(toggle_tcpcs=True, toggle_jntscs=False).attach_to(base)
    # print(robot_s.get_gl_tcp(component_name)) # robot_s.arm.jnts[-1]['gl_posq'] robot_s.get_gl_tcp(component_name)
    # base.run()
    ...
    for container in [container0, container1, container2, container3, container4]:
        container.set_pos(np.array([0, 0.6, 0]))
    ...
    for index in range(len(tcp)):
        ...
        while j < 20:
            ...
            goal = robot_s2.ik(component_name, goal_jaw_center_pos, goal_jaw_center_rotmat, approach1[-1])
            if goal is None:
