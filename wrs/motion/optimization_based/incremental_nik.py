...
class IncrementalNIK(object):
    ...
    def gen_linear_motion(self,
        ...
        for (pos, rotmat) in zip(pos_list, rotmat_list):
            ...
            rot = rm.quaternion_from_matrix(rotmat)
            rot = np.array([rot[1], rot[2], -rot[0], rot[3]])
            pose_matrix = np.concatenate([pos, rot])
            jnt_values = self.ur5e_arm.inverse(pose_matrix, False, q_guess=seed_jnt_values)
            if jnt_values is None:
                ...
                if not self.robot_s.is_jnt_values_in_ranges(component_name=component_name, jnt_values=jnt_values):
